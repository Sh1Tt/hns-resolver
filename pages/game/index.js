...
const Gamecanvas = () => {
    ...
    class Background {
        constructor(props) {
            ...
            this.dim = { 
                width: props.dim.width - props.gameSpeed, 
                height: props.dim.height
            };
            this.image = new Image();
            this.image.src = props.imageSrc;
            this.image.width = this.image.width * props.framesMax;
    ...
    useEffect(() => {
        ...
        const background = canvas.background = new Background({
            ...
            imageSrc: "/v1/sky.png",
            gameSpeed: speed * 0.025,
            framesMax: 1,
            framesHold: 1
        });

        const pathways = canvas.pathways = Array(sections)
            .fill(0)
            .map((_, i) => new Path({
                pos: {
                    x: 100 + (w * i),
        ...
        const gbns = canvas.gbn = Array(sections).fill(0)
            .map((_, i) => {
                return new GBN({
                    pos: {
                        x: (w * 0.875) + (w * i),
        ...
        const animate = () => {
            ...
            if (player.gbn === sections) {
                canvas.ctx.fillStyle = "white";
                canvas.ctx.font = "bold 48px Arial";
                canvas.ctx.fillText("You Win!", w / 2, h / 2);
